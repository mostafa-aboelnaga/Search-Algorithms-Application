...
def add_node(strategy, next_state, goal_state, action, current_node, compute_cost, heuristic):
    next_node = {}
    ...
    if strategy in ('UCS', 'Astar'):
        next_node['cost'] = current_node['cost'] + \
            compute_cost(current_node['direction'], action)
    if strategy == 'Greedy':
        next_node['h'] = heuristic(next_node['state'], goal_state)
    if strategy == 'Astar':
        next_node['f'] = next_node['cost'] + \
            heuristic(next_node['state'], goal_state)
    return next_node


def solve(strategy, initial_state, goal_state, get_actions, get_state, isgoal, compute_cost=None, heuristic=None):
    fringe = []
    visited = []
    ...
    while len(fringe) > 0:
        ...
        if isgoal(current_node['state'], goal_state):
            Solution = {}
        ...
        possible_actions = get_actions(
            current_node['state'])  # generate_actions
        for action in possible_actions:
            next_state = get_state(
                current_node['state'], action)  # apply_action
            next_node = add_node(strategy, next_state, goal_state,
